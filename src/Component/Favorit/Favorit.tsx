...
const Favorit = () => {
  ...
  useEffect(() => {
    ...
    if (storedUsers) {
      try {
        const parsedUsers: Users[] = JSON.parse(storedUsers); // Явное указание типа
        dispatch(setFavoritesUsers(parsedUsers));
      } catch (error) {
        console.error("Ошибка при парсинге пользователей из localStorage:", error);
      }
    }
  }, [dispatch]);
    ...
    <>
      <div className={styles.flexConteiner}>
        ...
        <div className={styles.group}>
          ...
          <input placeholder="Поиск" type="search" className={styles.input} value={search}  onChange={(e) => setSearch(e.target.value)}/>
        </div>
      </div>
